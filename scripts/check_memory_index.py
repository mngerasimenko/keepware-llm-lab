...
# Маркер и пробелы за ним: по ним считается колонка содержимого пункта, а от
# неё - где начинается блок кода. Прежде бралось `len(bullet.group(0))`, а это
# ВСЕГДА два: `\s` матчит один символ. Пункт «-   Инфраструктура» с вложенной
# строкой на шести пробелах терял её - гитхаб рисует список, а разбор молча
# выбрасывал строку и обвинял файл в том, что его нет в индексе.
BULLET = re.compile(r"^([-*+])( +)")
...
def parse_index_text(text):
    ...
    for lineno, line in enumerate(text.splitlines(), 1):
        ...
        if bullet:
            # По CommonMark содержимое начинается после маркера и пробелов за
            # ним, но пробелов считается не больше четырёх: пять и больше -
            # это уже блок кода внутри пункта, а содержимое начинается сразу
            # за маркером.
            gap = len(bullet.group(2))
...
def is_orphan_ok(path, rel, cache):
    ...
    но и не повод отменять весь прогон: считаем его обычной находкой. Явного
    сторожа тут не нужно - нечитаемый файл приходит из кэша пустой строкой, а
    в пустом тексте шапки нет и метки взяться неоткуда. Сторож стоял, и был
    мёртв: он возвращал ровно то же, что и код без него, то есть его мутация
    не могла быть поймана никаким тестом и печаталась бы «ВЫЖИЛА» вечно.
    """
...
#
# Взрыв закрывает ОГРАНИЧИТЕЛЬ floor в filename_tokens, а не состав класса
# имени. Пока его не было, «a.md/b.md/c.md...» оставался одним слитным
# прогоном, и каждый якорь заново шёл назад по всему предыдущему -
# посимвольный проход оказался квадратичным ровно там же, где до него была
# квадратичной регулярка. Замер на 40 КБ таких путей: 45.8 с без floor и
# 15.6 мс с ним.
#
# Точка в классе имени ОСТАЁТСЯ. Одно время её отсюда убрали - L6 её в именах
# памяти и так не допускает, - и цена сошлась (19.5 мс), но граница поехала:
# «prefix.user.md» стал читаться как упоминание нашего «user.md», а
# «release-1.2.md» - как «2.md». Это тот же ложный след, который проект ловит
# на «superuser.md», только заходящий с другой стороны. Правило оказалось не
# нужно для цены и портило смысл - редкий случай, когда правило стоит дороже
# обработчика, и тогда оно уходит.
...
def filename_tokens(text):
    ...
    Линейность держится на floor: скан назад не заходит левее конца
    предыдущего якоря, поэтому суммарная работа не больше длины текста.
    """
    found = []
...
def mentioned_in_raw_text(index_texts, *names):
    ...
    """
    # Граница слева та же, что у filename_tokens, и точка в неё входит:
    # «prefix.user.md» - другое имя, а не упоминание «user.md».
...
def check_files_alone(exact, index_name, cache, allow_globs,
    ...
    ветки «индекс не прочитан», нечитаемый индекс отменял и их - а
    заметка называла только L2 и L3. О невыполненной работе было сказано
    вслух, но не о той: читатель заключал, что имена и связи проверены, и
    коммит с нарушением L5 или L6 проходил, потому что код 2 хук не
    блокирует.
    """
...
def check(root, index_paths, allow_globs, index_name, file_map):
    ...
    if unreadable:
        notices.append(
            ...
            "L4, L5 и L6 от индекса не зависят и проверены"
            % len(unreadable)
        )
    ...
    # Тексты индексов и счётчик коротких имён нужны РАНЬШЕ цикла: без них
    # нельзя спросить, не съедена ли строка про файл незакрытым забором, а
    # спрашивать это надо до того, как выносится вердикт. Прежде оба
    # вычислялись ниже, и ветка про под-индекс успевала выдать блокирующую
    # ошибку с советом дописать строку, которая в индексе уже есть.
    ...
    def behind_unclosed(rel):
        """Индекс с незакрытым забором, где строка про файл всё-таки есть.

        Голое имя годится в поиске, только если оно однозначно: при двух
        «user.md» в разных папках оно указывало бы на чужую строку.
        """
        basename = os.path.basename(rel)
        unique = basename if namesake_counts[basename] == 1 else None
        return next(
    ...
    for rel in sorted(exact):
        ...
        if rel in index_rels:
            ...
            if is_orphan_ok(exact[rel], rel, cache):
                ...
                continue
            # Строка про него в индексе ЕСТЬ - просто её не успели
            # разобрать. Совет «добавьте строку» отправил бы дописывать
            # дубль вместо того, чтобы закрыть забор, а блокирующая ошибка
            # утверждала бы то, что человек своими глазами опровергает.
            cut_at = behind_unclosed(rel)
            if cut_at:
                kind, _path = incomplete[cut_at]
                notices.append(
                    "%s: строка про этот под-индекс в %s есть, но в разбор не "
                    "попала - там не закрыт %s, и всё ниже проверка не читает. "
                    "Закройте его в %s; пока он открыт, сказать, достижим "
                    "под-индекс или забыт, нельзя%s"
                    % (rel, cut_at, kind, cut_at, tail))
                continue
    ...
    # один раз на всех сирот, а не заново на каждую. Тексты индексов и счётчик
    # имён уже прочитаны выше - они понадобились раньше, до вердиктов.
    # Исключённый путь не читаем и здесь. Иначе нечитаемый файл под шаблоном
    # попадал в список непрочитанных, давал заметку и код 2 - то есть ключ,
    # обещающий «сюда проверка не смотрит», требовал починить файл, который
    # велено не трогать. Заодно и как источник упоминания он сомнителен: его
    # строки не разбираются, связи не проверяются.
    others = [(exact[other], other) for other in sorted(exact)
              if other.lower().endswith(".md") and other not in index_rels
              and not is_excluded(other, allow_globs)]
    wanted = set()
    for rel in orphans:
        wanted.add(rel)
    ...
    for rel in orphans:
        ...
        if cut_at:
            ...
            continue
        if rel in stranded:
            # Если недостижим не он, а перечисляющий его под-индекс, и
            # недостижим ПО ПРИЧИНЕ незакрытого забора - причина одна на
            # обоих, и называть надо её. Иначе человек получает два
            # блокирующих сообщения и оба советуют дописать строку, которая
            # в индексе уже есть.
            cut_at = behind_unclosed(stranded[rel])
            if cut_at:
                kind, _path = incomplete[cut_at]
                notices.append(
                    "%s: он перечислен в %s, а строка про сам %s в %s есть, но "
                    "в разбор не попала - там не закрыт %s. Закройте его в %s, "
                    "и станет видно всё, что за ним"
                    % (rel, stranded[rel], stranded[rel], cut_at, kind, cut_at))
                continue
...
def main(argv=None):
    ...
    # нашёл сотню битых связей в памятях портфеля, и ни одна не попадалась
