...
# Сколько битых [[ссылок]] печатать поимённо, прежде чем свернуть в число.
LISTED_WIKI_LINKS = 10
...
# справа: без него «user.mdx» откатывался до «user.md» и совпадал с другим
# файлом.
#
# Две проверки, а не одна: запрещаем продолжение имени (буква, цифра, дефис)
# и продолжение расширения (точка со значащим символом - «user.md.txt»), но
# НЕ запрещаем одиночную точку. Иначе «лежит в user.md.» в конце предложения
# переставало находиться вовсе - а связная проза с точками и есть обычный
# текст, который мы разбираем.
FILENAME_TOKEN = re.compile(r"[\w.\-/\\]+\.md(?![\w\-])(?!\.\w)", re.I)
...
def check(root, index_paths, allow_globs, index_name, file_map):
    ...
    # Список режем: в памяти, куда проверка приезжает не с первого дня, таких
    # ссылок бывают десятки, и стена одинаковых строк читается как поломка
    # проверки - тот же приём, что и с числом «за ним скрыто N файлов».
    ...
    if len(dangling) > LISTED_WIKI_LINKS:
        warnings.append("L4 таких же ссылок ещё %d - список обрезан"
                        % (len(dangling) - LISTED_WIKI_LINKS))
