...
def check(root, index_paths, allow_globs, index_name, file_map):
    ...
    for rel in orphans:
        ...
        if mentioned_in_raw_text(index_texts, rel, os.path.basename(rel)):
            ...
            # Самоупоминание отсеяно ещё при построении карты - второй
            # проверки тут не нужно, а мёртвая ветка делала тест на неё
            # пустым: он проходил, даже когда отсев из карты убирали.
