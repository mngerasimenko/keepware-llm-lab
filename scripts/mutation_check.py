...
def restore_interrupted():
    ...
    ours = resolve_ours(path) if path else None
    if ours is None:
    ...
    # пишем туда же, где искали. Имя из слепка остаётся только в сообщениях:
    # человеку надо видеть именно то, что написано в файле.
    path = ours
